  ...
  return action(async () => {
    ...
    if (existing) {
      ...
      if (createErr) {
        ...
        throw createErr.code === "23503"
          ? new AppError(
              "That stock isn't in the database yet.",
              "Import a statement containing it first, then add the holding."
            )
          : describeDbError(createErr, "Couldn't add the holding.");
      }
    ...
    if (error) {
      ...
      throw describeDbError(error, "Couldn't add the holding.");
    }
    revalidatePath("/");
  });
}
...
export async function updateHolding(
  ...
  return action(async () => {
    ...
    if (input.quantity !== undefined) {
      if (input.quantity <= 0) throw new AppError("Quantity must be positive.", "Enter a quantity greater than zero.");
      updateData.quantity = input.quantity;
    }
    if (input.avg_buy_price !== undefined) {
      if (input.avg_buy_price < 0) {
        throw new AppError("Average price cannot be negative.", "Enter a price of zero or more.");
      }
    ...
    if (error) {
      ...
      throw describeDbError(error, "Couldn't update the holding.");
    }
    revalidatePath("/");
  });
}
...
export async function deleteHolding(id: string): Promise<ActionResult> {
  return action(async () => {
    ...
    if (error) {
      ...
      throw describeDbError(error, "Couldn't remove the holding.");
    }
    revalidatePath("/");
  });
}
 ...
 */
export async function createCompanyWithHolding(formData: FormData): Promise<ActionResult<string>> {
  return action(async () => {
    const { supabase, user } = await getAuthUser();
    ...
    const { data: company, error: compErr } = await supabase
      ...
      .single();
    if (compErr || !company) {
      throw compErr?.code === "23503"
        ? new AppError(
            "That stock isn't in the database yet.",
            "Import a statement containing it first, then add the company."
          )
        : describeDbError(compErr, "Couldn't create the company.");
    }
    ...
    if (holdErr) throw describeDbError(holdErr, "The company was created, but its holding couldn't be saved.");
